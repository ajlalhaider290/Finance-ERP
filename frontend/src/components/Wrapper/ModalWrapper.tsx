...
type FormState<T = unknown> = {
  handleSubmit: (onSubmit: (data: T) => void, onInvalid?: () => void) => (e?: React.FormEvent) => void;
  formState: {
      ...
      (e?: React.FormEvent) => {
        ...
        if (onSubmit && form) {
          form.handleSubmit(onSubmit, () => {
            toast.error('Please review the required fields before submitting.');
          })(e);
        }
      },
      [form, onSubmit],
        ...
        <DialogContent
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar">
            <div className="relative min-h-[100px]">
              ...
              <form onSubmit={handleSubmit} className={cn('space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300', loading && 'pointer-events-none opacity-60')}>
                {children}
              </form>
            </div>
          </div>
          ...
          <DialogFooter className="justify-end items-end px-4 md:px-6 py-4 border-t bg-muted/5">
            <div className="flex items-center gap-3">
              <Button variant="outline" onClick={onClose} disabled={isSubmitting || loading} type="button" className="min-w-[80px]">
                {cancelText}
              </Button>
              {onSubmit && (
                <Button onClick={handleSubmit} disabled={isSubmitting || loading} type="submit" className="min-w-[100px]">
