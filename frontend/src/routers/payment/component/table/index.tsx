...
import { paymentGetColumns } from './columns';
...
export const PaymentTable: React.FC<PaymentTableProps> = memo(({
  fixedFilters,
  showToolbar = true,
  hiddenActions,
}) => {
  ...
  const {
    columnVisibility,
    tableHandlers
  } = useDataTableManager(PAYMENT_CONSTANTS.TABLE_CONFIG_KEY, paymentTableConfigDefault);

  // 1. Generate columns
  const columns = useMemo(() => paymentGetColumns(hiddenActions), [hiddenActions]);
  ...
  const {
    ...
    queryKey: [PAYMENT_CONSTANTS.QUERY_KEY, ...(fixedFilters ? [fixedFilters] : [])],
    fetchFn: getPayments,
    pageSize: 50,
    filterParsers: {
  ...
  return (
    <>
      <DataTableInfinite<PaymentIndex, unknown>
        data={flatData}
        columns={columns}
        ...
        entityName={PAYMENT_CONSTANTS.ENTITY_NAME}
