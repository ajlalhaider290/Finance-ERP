...
interface ExpenseCategoryRowActionsProps {
    row: {
        original: ExpenseCategoryIndex
    }
    hiddenActions?: string[]
}

export const ExpenseCategoryRowActions: React.FC<ExpenseCategoryRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = EXPENSE_CATEGORY_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.MODULE,
                        resource: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.MODULE,
                        resource: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: EXPENSE_CATEGORY_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
