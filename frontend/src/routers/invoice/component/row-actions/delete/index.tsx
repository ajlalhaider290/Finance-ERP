...
export const InvoiceDelete = memo(() => {
    ...
    const handleDelete = useCallback(async () => {
        try {
            ...
            toast.success(`${INVOICE_CONSTANTS.ENTITY_NAME} deleted successfully`);
            handleClose();
            queryClient.invalidateQueries({ queryKey: [INVOICE_CONSTANTS.QUERY_KEY] });
        } catch (error) {
            const message = axios.isAxiosError(error) && error.response?.data?.message
                ? error.response.data.message
                : `Failed to delete ${INVOICE_CONSTANTS.ENTITY_NAME}`;
            toast.error(message);
        }
    ...
    return (
        <DeleteConfirm
            ...
            curObjName={INVOICE_CONSTANTS.ENTITY_KEY}
            isDeleteLoading={isPending}
        />
    );
});

InvoiceDelete.displayName = 'InvoiceDelete';
