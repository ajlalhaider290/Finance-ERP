...
interface ReimbursementStatusHistoryRowActionsProps {
    row: {
        original: ReimbursementStatusHistoryIndex
    }
    hiddenActions?: string[]
}

export const ReimbursementStatusHistoryRowActions: React.FC<ReimbursementStatusHistoryRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.MODULE,
                        resource: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.MODULE,
                        resource: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: REIMBURSEMENT_STATUS_HISTORY_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
