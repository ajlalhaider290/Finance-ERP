...
import { reimbursementRequestTableConfigDefault } from '../../data/reimbursementRequestTableConfigDefault';
import { ReimbursementRequestIndex } from '../../interface';
import { getReimbursementRequests } from '../../service';
import { reimbursementRequestGetColumns } from './columns';
...
export const ReimbursementRequestTable: React.FC<ReimbursementRequestTableProps> = memo(({
  fixedFilters,
  showToolbar = true,
  hiddenActions,
}) => {
  ...
  const {
    columnVisibility,
    tableHandlers
  } = useDataTableManager(REIMBURSEMENT_REQUEST_CONSTANTS.TABLE_CONFIG_KEY, reimbursementRequestTableConfigDefault);

  // 1. Generate columns
  const columns = useMemo(() => reimbursementRequestGetColumns(hiddenActions), [hiddenActions]);
  ...
  const {
    ...
    queryKey: [REIMBURSEMENT_REQUEST_CONSTANTS.QUERY_KEY, ...(fixedFilters ? [fixedFilters] : [])],
    fetchFn: getReimbursementRequests,
    pageSize: 50,
    filterParsers: {
  ...
  return (
    <>
      <DataTableInfinite<ReimbursementRequestIndex, unknown>
        data={flatData}
        columns={columns}
        ...
        entityName={REIMBURSEMENT_REQUEST_CONSTANTS.ENTITY_NAME}
        renderToolbar={showToolbar ? () => <>
          {/* No Search form only Column Config */}
          <DataTableSettingsColumns
            variant="toolbar"
            columnVisibility={columnVisibility}
            defaultTableConfig={reimbursementRequestTableConfigDefault}
...
ReimbursementRequestTable.displayName = 'ReimbursementRequestTable';
