...
export const InvoiceDocumentCreateAction = memo(() => {
    const dispatch = useAppDispatch();
    return (
        <DataTableAction
            variant='default'
            size='default'
            label={`Create New ${INVOICE_DOCUMENT_CONSTANTS.ENTITY_NAME}`}
            onClick={() => dispatch(setSelectedObj({
                objKey: INVOICE_DOCUMENT_CONSTANTS.ENTITY_KEY,
                mode: 'form',
                label: `Create New ${INVOICE_DOCUMENT_CONSTANTS.ENTITY_NAME}`
            }))}
            permission={{
                ...
                action: INVOICE_DOCUMENT_CONSTANTS.PERMISSIONS.ACTIONS.CREATE,
            }} />
    )
})