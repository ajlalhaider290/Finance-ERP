...
interface VendorRowActionsProps {
    row: {
        original: VendorIndex
    }
    hiddenActions?: string[]
}

export const VendorRowActions: React.FC<VendorRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = VENDOR_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: VENDOR_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: VENDOR_CONSTANTS.PERMISSIONS.MODULE,
                        resource: VENDOR_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: VENDOR_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: VENDOR_CONSTANTS.PERMISSIONS.MODULE,
                        resource: VENDOR_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: VENDOR_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
