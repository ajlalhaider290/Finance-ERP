...
import { VendorDetail } from '../../../interface';

export const VendorViewDetails = ({ data }: { data?: VendorDetail }) => {
  return (
    <>
      {data && (
        <div className="flex flex-col h-full">
          ...
          <div className="col-span-2 text-sm">{(Array.isArray(data?.vendorName)
    ? (data?.vendorName as unknown[]).join(', ')
    : (typeof data?.vendorName === 'object' && data?.vendorName !== null
        ? JSON.stringify(data?.vendorName)
        : (data?.vendorName as string | number | boolean | null | undefined))) ?? '-'}</div>
        </div>
    ...
    : (typeof data?.address === 'object' && data?.address !== null
        ...
        <div className="grid grid-cols-3 py-3 border-b">
          ...
          <div className="col-span-2 text-sm">{data?.vendorsLabel || data?.entityId || '-'}</div>
        </div>
