...
import { paymentAllocationTableConfigDefault } from '../../data/paymentAllocationTableConfigDefault';
import { PaymentAllocationIndex } from '../../interface';
import { getPaymentAllocations } from '../../service';
import { paymentAllocationGetColumns } from './columns';
...
export const PaymentAllocationTable: React.FC<PaymentAllocationTableProps> = memo(({
  fixedFilters,
  showToolbar = true,
  hiddenActions,
}) => {
  ...
  const {
    columnVisibility,
    tableHandlers
  } = useDataTableManager(PAYMENT_ALLOCATION_CONSTANTS.TABLE_CONFIG_KEY, paymentAllocationTableConfigDefault);

  // 1. Generate columns
  const columns = useMemo(() => paymentAllocationGetColumns(hiddenActions), [hiddenActions]);
  ...
  const {
    ...
    queryKey: [PAYMENT_ALLOCATION_CONSTANTS.QUERY_KEY, ...(fixedFilters ? [fixedFilters] : [])],
    fetchFn: getPaymentAllocations,
    pageSize: 50,
    filterParsers: {
  ...
  return (
    <>
      <DataTableInfinite<PaymentAllocationIndex, unknown>
        data={flatData}
        columns={columns}
        ...
        entityName={PAYMENT_ALLOCATION_CONSTANTS.ENTITY_NAME}
        renderToolbar={showToolbar ? () => <>
          {/* No Search form only Column Config */}
          <DataTableSettingsColumns
            variant="toolbar"
            columnVisibility={columnVisibility}
            defaultTableConfig={paymentAllocationTableConfigDefault}
...
PaymentAllocationTable.displayName = 'PaymentAllocationTable';
