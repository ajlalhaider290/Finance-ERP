...
import { invoiceLineItemTableConfigDefault } from '../../data/invoiceLineItemTableConfigDefault';
import { InvoiceLineItemIndex } from '../../interface';
import { getInvoiceLineItems } from '../../service';
import { invoiceLineItemGetColumns } from './columns';
...
export const InvoiceLineItemTable: React.FC<InvoiceLineItemTableProps> = memo(({
  fixedFilters,
  showToolbar = true,
  hiddenActions,
}) => {
  ...
  const {
    columnVisibility,
    tableHandlers
  } = useDataTableManager(INVOICE_LINE_ITEM_CONSTANTS.TABLE_CONFIG_KEY, invoiceLineItemTableConfigDefault);

  // 1. Generate columns
  const columns = useMemo(() => invoiceLineItemGetColumns(hiddenActions), [hiddenActions]);
  ...
  const {
    ...
    queryKey: [INVOICE_LINE_ITEM_CONSTANTS.QUERY_KEY, ...(fixedFilters ? [fixedFilters] : [])],
    fetchFn: getInvoiceLineItems,
    pageSize: 50,
    filterParsers: {
  ...
  return (
    <>
      <DataTableInfinite<InvoiceLineItemIndex, unknown>
        data={flatData}
        columns={columns}
        ...
        entityName={INVOICE_LINE_ITEM_CONSTANTS.ENTITY_NAME}
        renderToolbar={showToolbar ? () => <>
          {/* No Search form only Column Config */}
          <DataTableSettingsColumns
            variant="toolbar"
            columnVisibility={columnVisibility}
            defaultTableConfig={invoiceLineItemTableConfigDefault}
...
InvoiceLineItemTable.displayName = 'InvoiceLineItemTable';
