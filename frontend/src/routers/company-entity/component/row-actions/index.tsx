...
interface CompanyEntityRowActionsProps {
    row: {
        original: CompanyEntityIndex
    }
    hiddenActions?: string[]
}

export const CompanyEntityRowActions: React.FC<CompanyEntityRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = COMPANY_ENTITY_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.MODULE,
                        resource: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.MODULE,
                        resource: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: COMPANY_ENTITY_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
