...
import { journalEntryTableConfigDefault } from '../../data/journalEntryTableConfigDefault';
import { JournalEntryIndex } from '../../interface';
import { getJournalEntries } from '../../service';
import { journalEntryGetColumns } from './columns';
...
export const JournalEntryTable: React.FC<JournalEntryTableProps> = memo(({
  fixedFilters,
  showToolbar = true,
  hiddenActions,
}) => {
  ...
  const {
    columnVisibility,
    tableHandlers
  } = useDataTableManager(JOURNAL_ENTRY_CONSTANTS.TABLE_CONFIG_KEY, journalEntryTableConfigDefault);

  // 1. Generate columns
  const columns = useMemo(() => journalEntryGetColumns(hiddenActions), [hiddenActions]);
  ...
  const {
    ...
    queryKey: [JOURNAL_ENTRY_CONSTANTS.QUERY_KEY, ...(fixedFilters ? [fixedFilters] : [])],
  ...
  return (
    <>
      <DataTableInfinite<JournalEntryIndex, unknown>
        data={flatData}
        columns={columns}
        ...
        entityName={JOURNAL_ENTRY_CONSTANTS.ENTITY_NAME}
        renderToolbar={showToolbar ? () => <>
          {/* No Search form only Column Config */}
          <DataTableSettingsColumns
            variant="toolbar"
            columnVisibility={columnVisibility}
            defaultTableConfig={journalEntryTableConfigDefault}
...
JournalEntryTable.displayName = 'JournalEntryTable';
