...
interface IntercompanyTransactionRowActionsProps {
    row: {
        original: IntercompanyTransactionIndex
    }
    hiddenActions?: string[]
}

export const IntercompanyTransactionRowActions: React.FC<IntercompanyTransactionRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = INTERCOMPANY_TRANSACTION_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.MODULE,
                        resource: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.MODULE,
                        resource: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: INTERCOMPANY_TRANSACTION_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
