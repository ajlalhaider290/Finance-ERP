...
import { approvalTaskTableConfigDefault } from '../../data/approvalTaskTableConfigDefault';
import { ApprovalTaskIndex } from '../../interface';
import { getApprovalTasks } from '../../service';
import { approvalTaskGetColumns } from './columns';
...
export const ApprovalTaskTable: React.FC<ApprovalTaskTableProps> = memo(({
  fixedFilters,
  showToolbar = true,
  hiddenActions,
}) => {
  ...
  const {
    columnVisibility,
    tableHandlers
  } = useDataTableManager(APPROVAL_TASK_CONSTANTS.TABLE_CONFIG_KEY, approvalTaskTableConfigDefault);

  // 1. Generate columns
  const columns = useMemo(() => approvalTaskGetColumns(hiddenActions), [hiddenActions]);
  ...
  const {
    ...
    queryKey: [APPROVAL_TASK_CONSTANTS.QUERY_KEY, ...(fixedFilters ? [fixedFilters] : [])],
    fetchFn: getApprovalTasks,
    pageSize: 50,
    filterParsers: {
  ...
  return (
    <>
      <DataTableInfinite<ApprovalTaskIndex, unknown>
        data={flatData}
        columns={columns}
        ...
        entityName={APPROVAL_TASK_CONSTANTS.ENTITY_NAME}
        renderToolbar={showToolbar ? () => <>
          {/* No Search form only Column Config */}
          <DataTableSettingsColumns
            variant="toolbar"
            columnVisibility={columnVisibility}
            defaultTableConfig={approvalTaskTableConfigDefault}
...
ApprovalTaskTable.displayName = 'ApprovalTaskTable';

export default ApprovalTaskTable;
