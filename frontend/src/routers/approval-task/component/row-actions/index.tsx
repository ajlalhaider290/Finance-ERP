...
interface ApprovalTaskRowActionsProps {
    row: {
        original: ApprovalTaskIndex
    }
    hiddenActions?: string[]
}

export const ApprovalTaskRowActions: React.FC<ApprovalTaskRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = APPROVAL_TASK_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: APPROVAL_TASK_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: APPROVAL_TASK_CONSTANTS.PERMISSIONS.MODULE,
                        resource: APPROVAL_TASK_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: APPROVAL_TASK_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: APPROVAL_TASK_CONSTANTS.PERMISSIONS.MODULE,
                        resource: APPROVAL_TASK_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: APPROVAL_TASK_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
