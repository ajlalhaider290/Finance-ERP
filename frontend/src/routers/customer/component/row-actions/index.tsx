...
interface CustomerRowActionsProps {
    row: {
        original: CustomerIndex
    }
    hiddenActions?: string[]
}

export const CustomerRowActions: React.FC<CustomerRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = CUSTOMER_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: CUSTOMER_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: CUSTOMER_CONSTANTS.PERMISSIONS.MODULE,
                        resource: CUSTOMER_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: CUSTOMER_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: CUSTOMER_CONSTANTS.PERMISSIONS.MODULE,
                        resource: CUSTOMER_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: CUSTOMER_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
