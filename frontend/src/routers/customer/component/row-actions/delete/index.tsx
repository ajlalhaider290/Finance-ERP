...
export const CustomerDelete = memo(() => {
    ...
    const handleDelete = useCallback(async () => {
        try {
            ...
            toast.success(`${CUSTOMER_CONSTANTS.ENTITY_NAME} deleted successfully`);
            handleClose();
            queryClient.invalidateQueries({ queryKey: [CUSTOMER_CONSTANTS.QUERY_KEY] });
        } catch (error) {
            const message = axios.isAxiosError(error) && error.response?.data?.message
                ? error.response.data.message
                : `Failed to delete ${CUSTOMER_CONSTANTS.ENTITY_NAME}`;
            toast.error(message);
        }
    ...
    return (
        <DeleteConfirm
            ...
            curObjName={CUSTOMER_CONSTANTS.ENTITY_KEY}
            isDeleteLoading={isPending}
        />
    );
});

CustomerDelete.displayName = 'CustomerDelete';
