...
interface ReimbursementDocumentRowActionsProps {
    row: {
        original: ReimbursementDocumentIndex
    }
    hiddenActions?: string[]
}

export const ReimbursementDocumentRowActions: React.FC<ReimbursementDocumentRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = REIMBURSEMENT_DOCUMENT_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.MODULE,
                        resource: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.MODULE,
                        resource: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: REIMBURSEMENT_DOCUMENT_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
