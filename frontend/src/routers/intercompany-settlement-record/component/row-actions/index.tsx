...
interface IntercompanySettlementRecordRowActionsProps {
    row: {
        original: IntercompanySettlementRecordIndex
    }
    hiddenActions?: string[]
}

export const IntercompanySettlementRecordRowActions: React.FC<IntercompanySettlementRecordRowActionsProps> = ({ row, hiddenActions = [] }) => {
    const dispatch = useAppDispatch();

    const { _meta } = row.original;
    const label = _meta?.label || '';
    const objKey = INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.ENTITY_KEY;
            ...
            {showView && (
                <DataTableAction
                    ...
                    permission={{
                        ...
                        action: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.ACTIONS.VIEW,
                    }}
                />
            )}
            {showEdit && (
                <DataTableAction
                    ...
                    permission={{
                        module: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.MODULE,
                        resource: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.ACTIONS.EDIT,
                    }}
                />
            )}
            {showDelete && (
                <DataTableAction
                    ...
                    permission={{
                        module: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.MODULE,
                        resource: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.RESOURCE,
                        action: INTERCOMPANY_SETTLEMENT_RECORD_CONSTANTS.PERMISSIONS.ACTIONS.DELETE,
                    }}
                />
            )}
        </div>
